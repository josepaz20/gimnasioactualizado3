...
function verAnular(idPago) {
    $.get('anular', {idPago: idPago}, setFormulario);
    bloqueoAjax();
}
...
function validarBusqueda() {
    ...
    var referencia = $.trim($("#referenciaFiltro").val());
    if (identificacion === '' && nombres === '' && apellidos === '' && razonsocial === '' && referencia === '') {
    ...
    if (identificacion !== '') {
        ...
        $.get('getabonadobyreferencia', {referencia: referencia}, setInfoCobros, 'json');
        bloqueoAjax();
    } else {
...
function confirmarAnulacionPago() {
    if (confirm(" DESEA ANULAR ESTE PAGO ? ")) {
        bloqueoAjax();
        return true;
    } else {
        return false;
