...
function buscarCliente() {
    if ($("#identificacion").val() !== '') {
        $.get('buscarcliente', {identificacion: $("#identificacion").val()}, setBusquedaCliente, 'json');
        bloqueoAjax();
    } else {
...
function setBusquedaCliente(datos) {
    if (parseInt(datos['error']) === 0) {
        ...
        $("#idServicioBusq").html(datos['selectservicioshtml']);
    } else {
        $("#idCliente").val(0);
        $("#tipocliente").val('');
        $("#cliente").val(0);
        $("#idServicioBusq").html('<option value="">Seleccione...</option>');
    }
    $("#idServicio").val('');
...
function getDetalleTipoOT() {
    if ($("#idTipoOT").val() !== '' && $("#idServicioBusq").val() !== '') {
        $.get('getdetalletipoot', {idTipoOT: $("#idTipoOT").val(), idServicio: $("#idServicioBusq").val()}, setInfoDetalleTipoOT);
        bloqueoAjax();
    } else {
        $("#divInfoTipoOT").html('');
        $("#divInfoTipoOT").hide('slow');
