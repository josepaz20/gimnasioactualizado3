...
class Module {
    ...
    public function getServiceConfig() {
        ...
        return array(
            'factories' => array(
                'Mikrotik\Modelo\DAO\PagoDAO' => function($sm) {
                    global $database;
                    if (is_null($database)) {
                        header("location: /josandro/login/login/cerrarSesion");
                        exit();
                    }
                    $dbAdapter = $sm->get($database);
                    $tabla = new Modelo\DAO\PagoDAO($dbAdapter);
                    return $tabla;
                },
                'Mikrotik\Modelo\DAO\MikrotikDAO' => function($sm) {
                    global $database;
                    if (is_null($database)) {
                        header("location: /josandro/login/login/cerrarSesion");
                        exit();
                    ...
                    $dbAdapter = $sm->get($database);
                    $tabla = new Modelo\DAO\MikrotikDAO($dbAdapter);
                    return $tabla;
                },
            ),
        );
