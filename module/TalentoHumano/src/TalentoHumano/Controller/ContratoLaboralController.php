...
use TalentoHumano\Formularios\ContratoLaboralForm;
use TalentoHumano\Modelo\Entidades\ContratoLaboral;
use TalentoHumano\Modelo\Entidades\Busqueda;

class ContratoLaboralController extends AbstractActionController {

    private $contratolaboralDAO;
...
//------------------------------------------------------------------------------    
    ...
    function getFormulario($accion = '', $idRegistro = 0) {
        $form = new ContratoLaboralForm($accion);
        if ($idRegistro != 0) {
            $registroOBJ = $this->getContratoLaboralDAO()->getRegistro($idRegistro);
...
//------------------------------------------------------------------------------
    ...
    public function getContratoLaboralDAO() {
        if (!$this->contratolaboralDAO) {
            $sm = $this->getServiceLocator();
            $this->contratolaboralDAO = $sm->get('TalentoHumano\Modelo\DAO\ContratoLaboralDAO');
        }
        return $this->contratolaboralDAO;
    }
...
//------------------------------------------------------------------------------
    ...
    public function indexAction() {
        ...
        return new ViewModel(array(
            'registros' => $this->getContratoLaboralDAO()->getRegistros($filtro)
        ));
    }
...
//------------------------------------------------------------------------------ / 
    ...
    public function registrarAction() {
        ...
        if ($request->isPost()) {
            ...
            if ($form->isValid()) {
                $session = $this->getInfoSesionUsuario();
                $registroOBJ = new ContratoLaboral($form->getData());
            
                try {
                    $this->getContratoLaboralDAO()->registrar($registroOBJ);
                    $this->flashMessenger()->addSuccessMessage('CONTRATO REGISTRADO EN GIMNASIO GYM-JAM');
                } catch (\Exception $ex) {
                    $msgLog .= "\n [ " . date('Y-m-d H:i:s') . " ]  -  REGISTRAR CONTRATO - RegistroController->registrar \n"
                            . $ex->getMessage()
                            . "\n *********************************************************************** \n";
                    $file = fopen("D:josandro.log", "a");
                    fwrite($file, $msgLog);
                    fclose($file);
                    $this->flashMessenger()->addErrorMessage('CONTRATO NO REGISTRADO EN GIMNASIO GYM-JAM');
                }
            } else {
    ...
    public function editarAction() {
        ...
        if ($request->isPost()) {
            ...
            if ($form->isValid()) {
                $registroOBJ = new ContratoLaboral($form->getData());
                try {
                    $this->getContratoLaboralDAO()->editar($registroOBJ);
                    $this->flashMessenger()->addSuccessMessage('CONTRATO ACTUALIZADO EN GIMNASIO GYM-JAM');
                } catch (\Exception $ex) {
                    $msgLog .= "\n [ " . date('Y-m-d H:i:s') . " ]  -  ACTUALIZAR CONTRATO - EmpleadoController->editar \n"
                            . $ex->getMessage()
                            . "\n *********************************************************************** \n";
                    $file = fopen("D:josandro.log", "a");
                    fwrite($file, $msgLog);
                    fclose($file);
                    $this->flashMessenger()->addErrorMessage('CONTRATO NO ACTUALIZADO EN GIMNASIO GYM-JAM');
                }
            } else {
                $this->flashMessenger()->addErrorMessage('LA INFORMACION REGISTRADA NO ES VALIDA');
            }
    ...
    public function eliminarAction() {
        ...
        if ($request->isPost()) {
            ...
            if ($idPregunta != 0) {
                ...
                try {
                    $this->getContratoLaboralDAO()->eliminar($idPregunta);
                    $this->flashMessenger()->addSuccessMessage('CONTRATO ELIMINADO DE  GIMNASIO GYM-JAM');
                } catch (\Exception $ex) {
    ...
    public function activarAction() {
        ...
        if ($request->isPost()) {
            ...
            if ($idRegistro != 0) {
                ...
                try {
                    $this->getContratoLaboralDAO()->activar($idRegistro);
                    $this->flashMessenger()->addSuccessMessage('CONTRATO LABORAL ACTIVADO EN GIMNASIO GYM-JAM');
                } catch (\Exception $ex) {
                    ...
                    fwrite($file, $msgLog);
                    fclose($file);
                    $this->flashMessenger()->addErrorMessage('CONTRATO LABORAL NO ACTUALIZADO EN GIMNASIO GYM-JAM');
                }
            } else {
                $this->flashMessenger()->addErrorMessage('LA INFORMACION NO ES VALIDA');
            }
...
//------------------------------------------------------------------------------  
    ...
    public function imprimirAction() {
        ...
        $plantillaHTML = file_get_contents('module/TalentoHumano/view/talento-humano/contrato-laboral/imprimircontrato.html');
        $registro = $this->getContratoLaboralDAO()->getServicioImprimir($idRegistro);
