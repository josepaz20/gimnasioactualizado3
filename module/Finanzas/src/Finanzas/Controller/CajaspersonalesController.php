...
use Dompdf\Dompdf;
use Finanzas\Formularios\MovimientocajaForm;
use Finanzas\Modelo\Entidades\Movimientocaja;
use Finanzas\Modelo\Entidades\Busqueda;

class CajaspersonalesController extends AbstractActionController {
...
//------------------------------------------------------------------------------    
    ...
    function getFormulario($accion = '', $idRegistro = 0) {
        $form = new MovimientocajaForm($accion);
        if ($idRegistro != 0) {
            $materialOBJ = $this->getFinanzasDAO()->getRegistro($idRegistro);
...
//------------------------------------------------------------------------------
    ...
    public function getFinanzasDAO() {
        if (!$this->finanzasDAO) {
            $sm = $this->getServiceLocator();
            $this->finanzasDAO = $sm->get('Finanzas\Modelo\DAO\FinanzasDAO');
        }
...
//------------------------------------------------------------------------------
    ...
    public function indexAction() {
        ...
        return new ViewModel(array(
            'registros' => $this->getFinanzasDAO()->getRegistros($filtro),
        ));
    }
